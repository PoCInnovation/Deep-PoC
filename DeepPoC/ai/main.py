...
def main_opencv(weights_path, file_path):
    img_treat = image_treatment()
    cnn_eyes = CNN()
    cnn_eyes.load_state_dict(torch.load("eye_model/10.0,1.0,1.0,10.0"))
    cnn_mouth = CNN()
    cnn_mouth.load_state_dict(torch.load("mouth_model/10.0,1.0,1.0,10.0"))
    ...
    while total < 10:
        ...
        if (len(faces) != 0):
            addition += yolo_test_utils.prediction_from_ai(img_treat, cnn_eyes, cnn_mouth)
            total += 1
