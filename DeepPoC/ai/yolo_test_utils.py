...
def prediction_from_ai(img_treat, cnn_eyes, cnn_mouth):
    output = 0
    if os.path.isfile("./eye_cv.jpeg"):
        input = img_treat.load_image_from_path("./eye_cv.jpeg").float().view(1, 3, img_width_down, img_height_down)
        output = cnn_eyes.forward(input).item()
    if os.path.isfile("./mouth_cv.jpeg"):
        input = img_treat.load_image_from_path("./mouth_cv.jpeg").float().view(1, 3, img_width_down, img_height_down)
        output += cnn_mouth.forward(input).item()
    print("predicted a :", output / 2)
    if (output / 2  >= 0.5):
        output = 1
    else:
        output = 0
