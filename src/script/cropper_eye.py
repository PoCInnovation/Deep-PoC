...
def crop_image(path, destination_path):
    ...
    path = path.split("/")
    im.save(destination_path + "eyes_" + path[-1])
    print("Cropped: {}".format(destination_path + "eyes_" + path[-1]))

def usage():
    print("\nThis script lets you crop the eyes of all images in a path.\n")
    print("Usage:\tcropper_eye.py [original_path] [destination_path] [Image_type]\n")
